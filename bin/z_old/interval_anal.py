...
def get_selected128_abs(Amaxn, Mmaxn, Pmaxn):
    ...
    M = np.r_[m_A, m_M, m_P]
    dpset = 0.
    nset = np.sum(np.abs(M) < dpset)
    i_128 = np.argsort(np.abs(M))[nset:]
    s_A = i_128[i_128 <= 95]; s_M = i_128[(i_128 > 95) & (i_128 <= 191)]; s_P = i_128[i_128 > 191]
    print '* selected A (0-based; n=%d):' % len(s_A), list(s_A)
    print '* selected M (0-based; n=%d):' % len(s_M), list(s_M)
    print '* selected P (0-based; n=%d):' % len(s_P), list(s_P)
    print "* selected 128 channel's <|d'| of max 10%>:", list(np.abs(M)[i_128])
    print "* selected 128 (0-based):", list(i_128)
    return i_128, np.abs(M)[i_128], np.abs(M)
...
def main():
    if len(sys.argv) < 3:
        print 'chg_anal.py <output file name> <file 1.csv> [file 2.csv] ...'
    ...
    # all_dps[0-based ch: 0 to 287] = {`iid`: [d', d', ...], ...}
    all_dps, Amaxn, Mmaxn, Pmaxn, Amaxr, Mmaxr, Pmaxr, dates = load_all_dps(sorted(files))
    # i_128: index of top-128, dp_abs_sorted: actual d' values of the 128
    i_128, dp_abs_sorted, dp_abs = get_selected128_abs(Amaxn, Mmaxn, Pmaxn)
    print 'Done.'
    pk.dump(dp_abs, open(fn, 'wb'))
