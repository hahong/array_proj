...
def get_movie_mat(fsorted, fout, tw=100000, iid_blank=101, stminfo=None, stimondur=STIMONDUR):
    # fsorted: sorted pk file. the "Chabo_20100818_RSVPNicole_A_001_sorted_data.pk" thing
    ...
    frate = np.zeros((nc, nw))         # frate[0-base channel, time window index] = firing rate
    lstm = np.zeros(nw).astype('int')  # lstim[time window index] = 0-based image id (e.g., Nat300_54 => 54)
...
def main(args):
    ...
    get_movie_mat('Chabo_20100818_RSVPNicole_A_001_sorted_data.pk',
                  'Chabo_20100818_RSVPNicole_A_001.movie.%dms.pk' % tw, tw=1000*tw, stminfo=stminfo)
