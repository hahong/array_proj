...
import numpy as np
import os
from mworks import data as mdata
from . import brreader, plxreader
...
class Merge:
    ...
    def __init__(self, mwk_filename, neu_filename):
        self.mwk_filename = None
        self.neu_filename = None
        self.mf = None
        self.nf = None
    ...
    def set_files(self, mwk_filename=None, neu_filename=None):
        ...
        if self.nf != None:
            ...
            self.nf = None
        # TODO: catch errors
        if mwk_filename != None:
            self.mf = mdata.MWKFile(mwk_filename)
            self.mf.open()
        if neu_filename != None:
    ...
    def mwk_extract(self, code_sent=None, code_recv=None):
        mf = self.mf
        if mf == None or not mf.valid:
            raise IOError, 'MWK file not ready.'

        if code_sent == None: code_sent = Merge.C_SENT
        if code_recv == None: code_recv = Merge.C_RECV
        ...
        recv = mf.get_events(codes=[code_recv], time_range=[start_time, mf.maximum_time])
    ...
    # -----------------------------------------------------------------------------
    def _debug_info(self, t_sent, t_recv, dat_sent, t_nsp, dat_nsp, plot_result=False):
        # TODO: clean up!!!
        # calculating onset of each event
        I_SENT = 0; I_LPBACK = 1; I_NSP = 2   # Mac -> ITC, ITC -> MAC, NSP
        ...
        for x, y in zip(dat_sent, dat_nsp):
            if x != y:
                if x - y == 128: mismatched_lessbad += 1   # MSB tends to be less reliable..
                else: mismatched_bad += 1
            else:
                matched += 1
    ...
    def _time_translation(self, sent, recv, nsp):
        ...
        # C0[0] : slope
        # C0[1] : intercept
        (C0, residues, rank, singular_values, rcond) = np.polyfit(nsp, sent, 1, full=True)
        return self._time_translation_core(C0, delay)


    def _time_translation_core(self, C0, delay):
        C0[1] += delay   # delay should be accounted. TODO: check is this fine. (truncation?)
    ...
    def merge(self, proc_wav=False, code_sent=None, code_recv=None,
        ...
        if mf.reverse_codec.has_key(Merge.C_MAGIC):
        ...
        if adj_reject is not None and amp_reject is not None:
            ...
            for ch in nf.chn_info:
                lthr = nf.chn_info[ch]['low_thr']; hthr = nf.chn_info[ch]['high_thr']
                if lthr == 0: thr0 = hthr
                else: thr0 = lthr
        ...
        if proc_wav or adj_reject is not None: proc_wav0 = True
        else: proc_wav0 = False
        ...
        if _force_timetransf is None:
            ...
            (t_sent, t_recv, dat_sent) = self.mwk_extract(code_sent=code_sent,
                                                          code_recv=code_recv)
            # (ITC->NSP, received timestamp words by NSP, corresponding index in
            #     `all_data` for `t_nsp` and `dat_nsp`, all other information
            #     including waveforms and electrode name, etc.)
            (t_nsp, dat_nsp, i_nsp) = self.neu_extract_timestamp()
            if len(t_sent) != len(t_recv):
                warnings.warn('Warning: length mismatch in ITC-loopbacks (sent=%d, recv=%d).' \
                        'Ignoring delay' % (len(t_sent), len(t_recv)))
                t_recv = t_sent

            dat_sent, dat_nsp, t_sent, t_recv, t_nsp, i_nsp = \
                    self._align(dat_sent, dat_nsp, t_sent, t_recv, t_nsp, i_nsp)
            if _debug: self._debug_info(t_sent, t_recv, dat_sent, t_nsp, dat_nsp)
        ...
        if _force_timetransf is None: toc_info[Merge.K_TSTAMP] = self.C_TSTAMP
        if proc_wav:   # if actual waveform data processing is requested,
            ...
            for eid in valid_elec_ids:
                ...
                wv_codes[eid] = mf.queue_code(code_metadata={'tagname': tagname})
        ...
        if _force_timetransf is None:
            ...
            for i in range(len(t_nsp)):
                ...
                ts_ev_blocks.append((long(np.round(t_nsp_mactime[i])), payload)) 
        ...
        while True:
            ...
            elec_id = the_data['id']            # actual electrode id
            spk_t = the_data['timestamp']       # onset of the spike in NSP
            ts = long(np.round(f(C, spk_t)))    # onset of the spike in MWorks time
            pos = the_data['file_pos']          # absolute data offset of the spike

            # bunch of rejection regimes
            if not elec_id in valid_elec_ids: continue
            if callback_reject != None and callback_reject(the_data, ts): continue
            if amp_reject != None:
