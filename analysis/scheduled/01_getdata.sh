...
rsync -avzuH --exclude='/.snapshot' --remove-source-files labuser@dicarlo4:data/blackrock_default/Tito*.* $PROJROOT/data/d002_Tito/neudat_NSP2/ 2>&1 | tee -a $LOGDIR/`date +%Y%m%d_%H%M%S`_Tito_neudat_dicarlo4.log &
...
rsync -avzuH --exclude='/.snapshot' --remove-source-files --ignore-existing labuser@dicarlo4:data/blackrock_log/Tito*.* $PROJROOT/data/d002_Tito/log/ 2>&1 | tee -a $LOGDIR/`date +%Y%m%d_%H%M%S`_Tito_log_dicarlo4.log &
wait
